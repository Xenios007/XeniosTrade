...
export function riskManagerPipelineNotes({ testMode = false, activeMode = false } = {}) {
  const lines = [
    ...
    'not relax or replace any ceiling, and it must match the riskPercent/leverage you actually chose. If the user message states a target',
    'profit per trade, that is the same kind of guidance: reason toward it with your own sizing when the setup and ceilings allow it, but',
    'never take a bad setup, or exceed a ceiling, just to reach it.',
    '',
