...
export function riskManagerPipelineNotes({ testMode = false, activeMode = false } = {}) {
  const lines = [
    ...
    '',
    'Reply with the JSON object requested at the end of the user message. Your finalConfidence is the `confidence` field (0-100) - it is',
    'not gated by code at any threshold; whatever you state is used as-is and shown to the account owner. There is no quantity field: the',
    'code derives the position from your `riskPercent` and your stop (Position Quantity ~ Maximum Loss / Stop Distance) and scales it to',
    'the wallet. `leverage` is used exactly as you return it - not a ceiling the code lowers from, whatever the role text above discusses',
    'in the abstract. `riskLevel` (LOW/MEDIUM/HIGH) is your own summary of how much you are risking here, driven by genuine confidence in',
    'this setup, not a fixed rule - see the riskLevel instruction in the user message. It is informational (shown to the account owner) and',
    'must match the riskPercent/leverage you actually chose. If the user message states a target profit per trade, that is the same kind of',
    'guidance: reason toward it with your own sizing when the setup allows it, but never take a bad setup just to reach it.',
    '',
    'The "Reference numbers" listed in the user message are NOT enforced in code: nothing clamps, widens, tightens or rejects your stop,',
    'target, risk% or leverage. They are shown so you can convert riskPercent into dollars and see what a rule-based bot would mechanically',
    'do here for contrast - you are not a bot, and your own judgment is what actually gets traded. The only checks left downstream are real',
    'exchange/account constraints, not opinions on the trade: the exchange minimum order size and the wallet margin actually available. Use',
    'only the evidence in the message; anything it does not contain (for example MFE/MAE percentiles, fees, funding, portfolio drawdown) is',
    'unavailable to you, so account for that uncertainty instead of inventing it. The backtest line, the exchange minimum order / margin',
    'lines and the open-positions line are real data when present.',
  ]
