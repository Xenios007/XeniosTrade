...
## Fixed leverage for real money (added 2026-09-21)

`execution.realFixedLeverage` (AI Settings -> Real money wallet -> "Fixed 10x leverage"; 0 = off, the default; bounded 0-10; only honoured in real mode, ignored on testnet). When on, `riskLimitsFor` returns `minLeverage = maxLeverage = fixedLeverage = N` (above the normal 5x ceiling on purpose: it is the account owner's choice), so the plan's leverage is **exactly N whatever the model returns**, and the rest follows from it: `margin = position / N`, the position is still `min(risk-sized position, margin cap x N)` (so a 36 USDT margin cap allows up to 360 USDT at 10x), the loss at the stop is `position x stop %`, and the liquidation-distance check applies at N (about 10% at 10x, stop must stay inside ~9%). Risk per trade (1% of the 1,000 USDT baseline) and every other real-money check are unchanged.

**Consequence to be aware of:** at 1x the margin cap itself limits the position; at 10x the same margin carries up to 10x the position, so the most a trade can lose is roughly `margin cap x 10 x stop %` (e.g. 36 x 10 x 1.5% = 5.4 USDT on a ~40 USDT wallet). The margin cap (`realMaxMarginUsdt`) is the knob that bounds it.

**The Risk Manager is told:** the user message states the fixed leverage, the largest position (margin cap x N), and, at the Analyst's stop and target, the position, margin, USDT loss (and % of the wallet's available balance), USDT gain and liquidation distance that follow; the system-prompt notes say leverage is fixed, to return N, and that this overrides the role text's "lowest sensible leverage". The exchange-minimum-order raise still applies but can only ever agree with N.
